...
NOTE: Does NOT start the bot – run.py handles start/stop lifecycle.
"""
...
def register_handlers(app: Client) -> None:
    ...
    # ── Callback queries (CAPTCHA first, then the rest) ───────────────────────
    ...
    # ── Text / prompt catch-all (must be last) ────────────────────────────────
    app.on_message(
        filters.text
        & filters.incoming
        & filters.private
...
async def post_init(app: Client) -> None:
    """Called by run.py after app.start() completes."""
    try:
...
def main() -> Client:
    """Create the Pyrogram client and register all handlers. Does NOT start it."""
    app = create_bot()
